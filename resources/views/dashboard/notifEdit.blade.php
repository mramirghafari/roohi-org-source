...
<head>
    ...
    <meta content="width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0"
        name="viewport" />
    <title>ویرایش اعلان - روحی بات</title>
...
<body>
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <div class="row">
                            <div class="col-12 col-md-8">
                                <h4 class="py-3 mb-4">
                                    <span class="text-muted fw-light">کاربران / اعلانات / </span>
                                    ویرایش اعلان: {{ $notif->title }}
                                </h4>
                            </div>
                            <div class="col-12 col-md-4 d-flex justify-content-end align-items-center">
                                <a href="{{ route('users.sendNotif') }}" class="btn btn-secondary">بازگشت به اعلانات</a>
                            </div>
                        </div>
                        @if (session('success'))
    ...
    <div class="col-12">
        <div class="card">
            <form method="POST" action="{{ route('notif.update', $notif->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="card-header">
                    <h5 class="card-title m-0">ویرایش جزئیات اعلان</h5>
                </div>
                <div class="card-body">
                    ...
                    <div class="row mb-3 g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input aria-label="تیتر اعلان" class="form-control" id="title" name="title"
                                placeholder="تیتر اعلان" type="text" value="{{ $notif->title }}" />
                        </div>

                        <div class="col-12">
                            <label class="form-label mb-0" for="send_to">ارسال به:</label>
                            <select aria-label="ارسال به" class="form-select" id="send_to" name="send_to">
                                <option value="">انتخاب کنید...</option>
                                <option value="1" @selected($notif->usersGroup == 1)>همه کاربران</option>
                                <option value="2" @selected($notif->usersGroup == 2)>کاربران VIP</option>
                                <option value="3" @selected($notif->usersGroup == 3)>کاربران با اشتراک کمتر از 10 روز
                                </option>
                                <option value="4" @selected($notif->usersGroup == 4)>کاربران با اشتراک کمتر از 7 روز
                                </option>
                                <option value="5" @selected($notif->usersGroup == 5)>کاربران با اشتراک کمتر از 3 روز
                                </option>
                                <option value="6" @selected($notif->usersGroup == 6)>کاربران با اشتراک کمتر از 2 روز
                                </option>
                                <option value="7" @selected($notif->usersGroup == 7)>کاربران رفته</option>
                        ...
                        <div class="col-12">
                            ...
                            <textarea aria-label="محتوا" class="form-control" id="content" name="content" placeholder="متن کامل پست..."
                                rows="10">{{ $notif->content }}</textarea>
                            @error('content')
                        ...
                        <div class="col-12">
                            ...
                            <select aria-label="ارسال SMS" class="form-select" id="sms" name="sms">
                                <option value="0" @selected($notif->sms == 0)>غیرفعال</option>
                                <option value="1" @selected($notif->sms == 1)>پیامک ارسال شود.</option>
                            </select>
                        </div>

                    </div>
                    <button class="btn btn-primary waves-effect waves-light" type="submit">ویرایش اعلان</button>
